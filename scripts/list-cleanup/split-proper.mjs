...
const read = (n) => {
  try { return fs.readFileSync(path.join(listsDir, `${n}.txt`), "utf8").split(/\r?\n/).map((l) => l.trim()).filter(Boolean); }
  catch { return []; }
};
...
const buckets = { confirmedCity: [], place: [], org: [], "given-name": [], surname: [], leftover: [] };
for (const w of read("keyword")) {
  const lc = w.toLowerCase();
  const doc = nlp(w);
  if (citySet.has(lc)) buckets.confirmedCity.push(w);
  else if (doc.has("#Place") || doc.has("#City") || doc.has("#Country") || doc.has("#Region")) buckets.place.push(w);
...
if (apply) {
  const writeNew = (n, arr) =>
    fs.writeFileSync(path.join(listsDir, `${n}.txt`), Array.from(new Set(arr)).sort((a, b) => a.localeCompare(b)).join("\n") + "\n");
  ...
  console.log("\nAPPLIED. keyword.txt now:", new Set(buckets.leftover).size,
    "| given-name", new Set(buckets["given-name"]).size,
    "| surname", new Set(buckets.surname).size,
    "| place", new Set(buckets.place).size,
    "| organization", new Set(buckets.org).size,
    "| confirmed cities dropped (already in city.txt):", new Set(buckets.confirmedCity).size);
}
