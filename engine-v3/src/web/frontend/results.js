...
function resultsLoaded(results) {
  ...
  for (let i = 0; i < results.images.length; i++) {
    if (results.images[i].link == undefined) continue;

    $("#images").append(
      `<a href="${results.images[i].link}" class="image"><img src="${results.images[i].image}"/></a>`,
    );
  }
...
function makeArt() {
  ...
  if (pageSearch == undefined || pageSearch == null) pageSearch = JSON.stringify({});
