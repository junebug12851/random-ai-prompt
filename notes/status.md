...
| **Mobile on-device (Detox — release APK on a real Android runtime)** | the 1000-prompt max load, on real frame stats |
...
**Closed in 2.60.0 — the last two open items of the mobile campaign:**
- ~~The 1000-prompt load is UNVERIFIED on mobile~~ → **verified on a device.** `targets/mobile/e2e/` +
  `.detoxrc.js` drive the **release** APK with Detox (local `rap_phone` AVD; CI `android-device` job) and
  assert **scaling against a same-session baseline** — 20 prompts vs 1000: 50× the rows must not cost 50×
  the memory or collapse the frame rate — using the platform's own accounting (`dumpsys gfxinfo` /
  `meminfo`), not a stopwatch in the test. See [`plans/testing.md`](plans/testing.md).
