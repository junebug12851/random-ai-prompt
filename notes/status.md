...
| **Mobile on-device (Detox — release APK on a real Android runtime)** | 🔴 **RED — and correctly so:** the app misses the 1000-prompt promise on a device (see below) |
...
**🔴 OPEN, P1 — the 1000-prompt promise is NOT met on a real device. The new gate found it.**

The Detox harness (below) ran the release APK on a real Android runtime, and the app **failed its own
headline promise**:

| Roll | Result (CI emulator, KVM, release build) |
|---|---|
| 20 prompts | **2218 ms** · 123 MB · p90 150 ms · 97% janky frames (software GPU — noise, cancels out) |
| 1000 prompts | **never rendered `"1000 generated"` within 180 s** — timed out |

Linear scaling from the baseline predicts ~110 s; it exceeded 180 s, so the cost is **super-linear in
N** somewhere. The engine is *not* the suspect: `nodeLoader` **and** `metroLoader` both roll 1000 prompts
in ~158 ms (0.16 ms/prompt), and mobile's generate path does ONE `setResults` for the whole batch. That
leaves the render path (FlashList v2 recycling / the row tree) or a Hermes-side cost the desktop engine
measurement can't see. **Not yet diagnosed — it needs an on-device profile, not another guess.**

Note this **retires a previous conclusion**: the 2026-07-11 session blamed FlashList's *web* renderer for
the same symptom in the react-native-web proxy and called the device "unverified". The device now shows
the same pathology, so the proxy may have been telling the truth. *The harness earned its keep on its
first real run.*

Until it's fixed, do not describe the 1000-prompt figure as verified on mobile.
