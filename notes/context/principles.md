...
- **Match the existing module shapes.** Dynamic prompts are `export default function (settings,
  imageSettings, upscaleSettings)`; settings are `export default {…}`; the loader contracts in `src/`
  and `prompt-modules/` expect those. Keep new files consistent so the scanners/loaders keep working.
- **Keep the two entry points in sync conceptually.** The web UI generates by shelling out to the CLI,
  so a CLI flag and the server feature that uses it must agree.
