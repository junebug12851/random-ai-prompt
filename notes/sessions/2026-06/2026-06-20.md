...
and FIXED the breakage it introduced: 18 block files still referenced the old tokens
...
`prompt-modules/block.js` + `core/stages/block.js` now tests `startsWith("d/")` (was `d-`);
...
`gatedBlocks` emptied, `#danbooru` ungated since it now draws the mode-aware `d/general`).
...
(`danbooru.js`, `applyArgs.js`, both block person-replacers). danbooru files nested
