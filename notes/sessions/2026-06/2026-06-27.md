...
Kicked off a large block overhaul (owner request: strip "masterpiece/best quality" filler,
...
- `src/core/stages/block.js` — `{#name NN%}` parsing threaded through `expandGen`/`pickFrom`/`run`
...
- **`nsfw` sidecar flag** = hard-hide when the NSFW switch is off. `getBlocks({ includeAdult })` drops
  ...
  (`src/core/stages/block.js`) gates on `loader.readBlockMeta(key)?.nsfw === true ||
  isGatedBlock(key)` — so flagged generators are unpickable, excluded from `{#any}`/groups, and resolve
