...
- **Second pass (owner follow-ups):** (1) **dark mode** — captures now run in a dark Playwright context
  ...
  crop** — the block-menu shot is now full-page like the rest, and the GIF is the full 1025×768 frame;
  (3) **autocomplete visible** — the GIF typing now pauses ~140ms per keystroke so CodeMirror's token
  autocomplete popup renders and is captured. The full-frame diff encode was too slow first pass
  (pngjs decode + re-mapping every pixel of 72 full-HD frames), so the encoder now maps only the
  **changed** pixels per frame (`nearestColorIndex`), the rest transparent — full-frame GIF ~760 KB.
