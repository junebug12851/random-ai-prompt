...
project reader: `currentDeploy.state = current`, deploy `ready`), so the site is healthy. Switched the
README badge to the official `api.netlify.com/.../deploy-status` badge, which tracks the *production*
deploy state (ready → success). Follow-up worth considering: the dual git-build + Actions deploy is
redundant; picking one would stop the canceled build records entirely.
