...
badge, but it showed **"no deploys"** — because production deploys come from the `netlify-deploy.yml`
Actions workflow (`netlify deploy` via CLI), not Netlify's git integration, so Netlify's branch-scoped
badge finds nothing. Both Netlify-native badges misreport this CLI-deploy setup. Final fix: point the
badge at the **actual deploy mechanism** — a GitHub Actions workflow-status badge for
`netlify-deploy.yml` on `main` (verified: renders `passing`). Follow-up worth considering: the dual
git-build (`netlify.toml`) + Actions deploy is redundant; picking one would stop the canceled records.
