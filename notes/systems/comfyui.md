...
```
ComfyUI (Python nodes) ──HTTP──▶ app backend (/api/prompt, /api/prompt/catalog, /api/rewrite)
...
- **Point-at-running-app** (the chosen model): the nodes call `http://127.0.0.1:4173` (the desktop /
  ...
  a per-node `server_url` widget. The app need not be running when ComfyUI starts — dropdowns fill in and
  generation works once it's reachable.
...
| `client.py` | The only thing that talks to the app backend — `generate` / `rewrite` / `catalog`, base-URL resolution, friendly errors. Stdlib `urllib`. |
...
- **🎲 Prompt Rewrite** (`RandomAIPromptRewrite`) — auto-fix / keyword-translate via a text provider (BYOK) → `/api/rewrite`.
...
The nodes talk only to the **local app backend** (localhost) and, for the rewrite node, the user's chosen
text provider with a **BYOK** key — the same flows the app already discloses. No new third-party data
flow, so the legal pages are unchanged (re-checked 2026-07-07).
