...
1. **CLI generate** (`index.js`) — the workhorse. Parses argv, optionally loads variation/reroll/
...
2. **Web UI** (`server.js`) — Express on `serverSettings.port` (7861). Serves Pug pages + a JSON API +
   the `output/` images, and **spawns the CLI** for any actual generation. It reads generation progress
   by HTTP-polling the CLI's progress server, and merges that with its own `execAppOngoing` flag.
3. **The browser** — `web/frontend/*` (jQuery-era classic scripts) talking to that JSON API.

`common.js` is imported by both `index.js` and `server.js` and is where the shared `run()`,
`processBatch()`, `upscale()`, and the settings accessors live.

## Boot order (and why `chdir.js` is first)

Both entry points ultimately import `common.js`, whose **first** import is `./chdir.js`
(`process.chdir(import.meta.dirname)`). This matters because ES-module imports are evaluated in source
order *before* any top-level statement runs: `loadSettings.js` reads `./user-settings.json` at import
time, so the chdir has to happen via an earlier-imported module, not via an inline statement in
`common.js`. After chdir, `loadSettings.js` builds the merged settings (defaults ⊕ user-settings ⊕
legacy migration), and `common.js` exposes `settings()` returning the live object.

## The prompt-module pipeline

`processBatch()` takes `settings.prompt` and runs it through `settings.promptModules` in order. Each
named module is a file in `prompt-modules/`, loaded by config-driven path:

```
prompt → expansion → dynamic-prompt → expansion → dynamic-prompt → prompt-salt → list → cleanup
```

- **expansion** (`<name>`): splice in `expansions/name.txt`.
- **dynamic-prompt** (`#name`): expand by calling `dynamic-prompts/name.js`'s default export. Supports
  nesting (re-runs up to 10x), `-v1` variants (`dynamic-prompts/v1/`), `user-` prefixed user-submitted
  prompts, and auto-appended `#fx` / `#artists`. Danbooru keyword substitution is applied.
- **prompt-salt** (`{salt}`): inject a random or incrementing number (a subseed alternative).
- **list** (`{name}`): pull a random line from `lists/name.txt`, with emphasis/editing/alternating
  randomization (`helpers/randomEmphasis|Editing|Alternating.js`) and once-only depletion.
- **cleanup**: collapse stray spaces/commas.

Two loaders make this work synchronously: `prompt-modules/dynamic-prompt.js` and `common.js` both use
...
`src/promptFilesAndSuggestions.js` scans `dynamic-prompts/` (+ `v1/`, `user-submitted/`) and splits the
...
(`helpers/saveImage.js`) and optionally upscales (`helpers/imageUpscaler.js`). Animations are stitched
into an APNG (`helpers/makeApng.js`, using `crc`).

`web/backend/indexImages.js` builds an in-memory index from every `output/*.json`: a keyword→files map
...
diff to `user-settings.json`. The CLI applies presets (`presets/*.json`) and per-flag overrides through
