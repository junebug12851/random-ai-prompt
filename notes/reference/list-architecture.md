...
gated exactly like lists. Groups are pure unions — there is no runtime content filtering; SFW/NSFW are
separate preprocessed files (`danbooru-sfw` reads `danbooru/d-sfw/*`, split from the full `danbooru/d/*` at
build time by `scripts/list-cleanup/split-danbooru-nsfw.mjs` and the CSV build script). Groups may include
groups up to `MAX_GROUP_DEPTH` (3) levels with a cycle guard. Groups live in their
