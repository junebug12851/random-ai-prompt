...
- **`engine/core/rng.js`** — a real, self-contained PRNG.
...
- **`engine/helpers/random.js`** — the pipeline's draw helpers (`randomFloat` / `randomInt` / `sample` /
...
- **`engine/core/dpl/rng.js`** — the DPL renderer's `ctx.rng` seam delegates to `randomFloat`, and now
  also exposes `float()`. The `weightedSampleN` leak was fixed to draw from `ctx.rng`.

## How the engine uses it

`engine/core/engine.js` installs the ambient `Rng` for a generation:
...
`settings.seed` flows through the GUI facade (`targets/web/frontend/lib/promptEngine.js`) automatically because it
...
- **Never** use lodash `_.random` / `_.sample` / `_.shuffle` (or a bare `Math.random`) in engine code
  or in a generator — they bypass the ambient source and can't be seeded. Use `engine/helpers/random.js`
