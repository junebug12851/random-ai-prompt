...
- **Per-prompt engine cost** — create a throwaway script that imports `createEngine` from
  `engine/core/engine.js` + `nodeLoader` from `engine/core/nodeLoader.js`, then time `engine.generate({ prompt })`
...
- **Build + bundle** — `npm run test:perf` (builds the SPA, then `scripts/check-bundle-size.mjs` gzips
  every `targets/web/dist/assets/*.js` against the 900 KB budget). Vite also prints per-chunk raw+gzip sizes.
...
- `nodeLoader.readListLines(name)` rebuilt the entire list-name set on **every call** — `groupListDirs()`
  ...
  every prompt.
- The reserved **`keyword` wildcard** (`engine/listResolve.js`) is a union of *all* general vocabulary: it
...
**Manage tab** — already reads through a **separate** `targets/web/frontend/lib/runtimeLoader.js` (a disk-snapshot
...
- **Chunking** (`targets/web/vite.config.js` `advancedChunks`) already isolates `react`, `intl`, `lodash`, and the
