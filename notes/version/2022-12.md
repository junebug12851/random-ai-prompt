...
randomization engine, a large CC-licensed list library, dozens of blocks, JSON settings, and
...
**beach** block (user-submitted, then tuned) and a convention for **user-submitted** dynamic
...
expansions to also expand **into blocks**. Added **vehicle** and **ship** blocks (and
...
landed the project at **2.0**. Added **legacy support for v1 blocks** (use the old ones by
...
### 2022-12-15 → 12-17 — Server split + JSON settings + the anime toggle + many blocks
...
**anime / non-anime** switch for generation. A burst of new blocks arrived — **cave, sticker,
...
Added **colors, hairstyle, and emotions** lists; added **princess** and **school-room** blocks and
...
the **pixel-art** expansion, the **castle** and **room** blocks, and made blocks
**combine with `AND`** and **stack**. Fixed yargs turning an empty prompt/negative-prompt into the string
`"true"`, and added a way to prefix prompts even when a block replaces the whole prompt.
...
ones. First blocks: **candlelight (+ danbooru variant), city, landscape, house** and a
greatly-improved **person** generator. Core blocks were **prefixed with `_`**, and early bugs were
