...
**Open (for owner review):** full-pipeline async. The pure-CPU render path is intentionally left
synchronous (it drives the instant live preview and is called during React render in several places);
"async" is provided at the batch boundary. If a fully-async engine API is wanted, that's a scoped
follow-up.
