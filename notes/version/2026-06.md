...
- **`{#artists}` is framed** ("by …") when the setting is on (`data/blocks/prompt/artists.js`),
  via a post-process so the raw `artistRepeater` (and its unit tests) are untouched.
- **New `{#styles}` generator** (`data/blocks/prompt/styles.js` + `.dpl`/`.json`) emits 1–2
...
- **Full file-watch hot reload.** `/api/manage/watch` is now a scoped SSE stream — `data`
  (lists/blocks → live catalog refresh), `output` (the gallery feed), and `settings`
...
- **`gui/src/components/Manage.jsx` (new):** the Manage tab — Generate's two-pane skeleton, but a
  content manager. Left: the **real nested folder tree** of both roots (Blocks = blocks,
...
- **`gui/server/manageFs.js` (new):** the local-mode content backend — traversal-guarded fs helpers
  over `data/lists` + `data/blocks`: a full-catalog **snapshot** builder, a folder-tree
...
- `src/core/stages/block.js`: the auto-`{#fx}` / `{#artists}` append was moved to run **before**
...
- **This completes the block content refactor across all five categories** (scene, fragment,
...
- **Token argument** — `{#great-bridge 25%}` runs the generator at 25% intensity (1–100; `0%`→`1%`;
  ...
  `src/core/stages/block.js` and handed to the generator as a 4th argument
...
- **`nsfw` sidecar flag (hard-hide).** A generator whose `.json` sidecar carries `nsfw: true` is treated as
  **non-existent when the NSFW switch is off** — hidden from the picker (`getBlocks({ includeAdult })` drops
  ...
  (`block.js`) now gates on `sidecar.nsfw === true || isGatedBlock(name)`, so it's excluded from
...
[`core/stages/block.js`](../../engine/core/stages/block.js)), so every existing reference still
...
Notes-only. Added [`reference/blocks.md`](../reference/blocks.md) — a catalog of all 113
...
- The entry points (`index.js`, `server.js`, `common.js`) and the `src/` loaders were hand-converted.
  ...
  (blocks, prompt modules) kept synchronous loading via `createRequire(import.meta.url)` —
