...
thumbnails (wide/tall by aspect), a keyword search over prompt/DPL/provider, and a click-through detail
view showing the whole record with copy / open / reveal / **delete** (which removes the image *and* its
sidecar). Implemented on the dev-server side as `POST /api/image` (now accepts `meta` and writes the
