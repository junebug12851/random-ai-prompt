...
28 SPA tests, SPA build; **lint warnings dropped 140 → 18**. Trivial dead-code remnants left as a follow-up
(the loaders' now-unreachable `expansionNames`/`readExpansion`/glob and `fakeLoader`'s expansion support;
`settings.expansionFiles`). No `VERSION` bump pending the release decision.
