...
function dataRefreshComplete(data) {
  ...
  const progressPercentSteps =
    progressTotalSteps == 0 ? 100 : Math.round((progressCurStep / progressTotalSteps) * 100);
  ...
  const progressPercentImg =
    progressTotalImg == 0 ? 100 : Math.round(((progressCurImg + 1) / progressTotalImg) * 100);
  ...
  const progressPercentPrompts =
    progressTotalPrompts == 0 ? 100 : Math.round((progressCurPrompt / progressTotalPrompts) * 100);
...
async function loadResults() {
  ...
  for (let i = loadedPrompts; i < results.prompts.length; i++, loadedPrompts++) {
    try {
      $("#prompts").append(
        `<p class="prompt">${results.prompts[i].replaceAll("<", "&lt;").replaceAll(">", "&gt;")}</p>`,
      );
    } catch (err) {}
  }
  ...
  for (let i = loadedImages; i < results.images.length; i++, loadedImages++) {
    if (results.images[i] == undefined) continue;

    $("#images").append(
      `<a href="${results.images[i]}" class="image" rel="noopener noreferrer" target="_blank"><img src="${results.images[i]}"/></a>`,
    );
  }
...
function toggleDetailedProgress() {
  ...
  if ($("#minimal-progress-bar").is(":visible")) $(".minimal-expand").text("Expand Progress");
  else $(".minimal-expand").text("Collapse Progress");
}
...
$(document).ready(function () {
  ...
  if (encodedUrl != undefined) backUrl = decodeURIComponent(encodedUrl);
