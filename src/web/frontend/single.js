...
function onPromptSelectionChange() {
  ...
  if (selectedOption == "prompt") $("#keywords").text(imageData.prompt);
  else if (selectedOption == "negative") $("#keywords").text(imageData.negative_prompt);
  else if (selectedOption == "data") $("#keywords").text(JSON.stringify(imageData, null, 4));
...
Generated using [RandomAIPrompt](https://github.com/junebug12851/random-ai-prompt)`;
    ...
    if (imageData.origPrompt) txt += ` with keywords: ${imageData.origPrompt}`;
...
Generated using RandomAIPrompt (https://github.com/junebug12851/random-ai-prompt)`;
    ...
    $("#keywords").text(txt);
  } else if (selectedOption == "original") $("#keywords").text(imageData.origPrompt);
  else if (selectedOption == "post") $("#keywords").text(imageData.origPostPrompt);
  else if (selectedOption == "rnd") $("#keywords").text(imageData.origRandomPrompt);
  else if (selectedOption == "cmd") $("#keywords").text(imageData.cmd);
...
function completePage() {
  ...
  if (imageData.cmd) $("#prompt-selection").append('<option value="cmd">Command</option>');
  ...
  if (
    imageData.variationOf != undefined ||
  ...
  if (imageData.variationOf != undefined) $("#type-cell").text("Variation Image");
  else if (imageData.rerollOf != undefined) $("#type-cell").text("Re-rolled Image");
  else if (imageData.animationFrameOf != undefined)
    $("#type-cell").text("Animation Frame" + frameNumber);
  else if (imageData.isAnimation != undefined) $("#type-cell").text("Animation");
  else if (imageData.upscaleOf != undefined) $("#type-cell").text("Upscaled Image");
  else if (imageData.upscaleOf == undefined && imageData.isUpscale == true)
    $("#type-cell").text("Upscaled Replacing Base");
  else $("#type-cell").text("Base Image");
  ...
  if (imageData.isAnimation != undefined) {
    ...
    if (imageData.animationFrames) $("#frame-count-cell").text(imageData.animationFrames.length);
    else $("#frame-count-cell").text(0);
  ...
  if (
    imageData.animationFrameOf == undefined &&
    imageData.isAnimation == undefined &&
  ...
  if (imageData.upscales) $("#delete-upscales").show();
  ...
  // Then show it here
  if (
    (imageData.seed_resize_from_w > -1 && imageData.seed_resize_from_w != imageData.width) ||
  ...
  const restoreFacesModel =
    imageData.face_restoration_model == null || imageData.face_restoration_model == "None"
      ? ""
      : ` (${imageData.face_restoration_model})`;
  ...
  $("#data-file-cell").html(
    `<a href="${imageData.dataPath}" rel="noopener noreferrer" target="_blank">${imageData.dataPath}</a>`,
  );
  $("#image-file-cell").html(
    `<a href="${imageData.imgPath}" rel="noopener noreferrer" target="_blank">${imageData.imgPath}</a>`,
  );
...
function rerollPrompt(isSelect) {
  ...
  if (selectedOption == "prompt") fieldName = "prompt";
  else if (selectedOption == "original") fieldName = "origPrompt";
  else if (selectedOption == "post") fieldName = "origPostPrompt";
  else if (selectedOption == "rnd") fieldName = "origRandomPrompt";
...
async function deleteFile() {
  ...
  if (imageData.variationOf != undefined) backUrl = `/single?name=${imageData.variationOf}`;
  else if (imageData.rerollOf != undefined) backUrl = `/single?name=${imageData.rerollOf}`;
  else if (imageData.animationFrameOf != undefined)
    backUrl = `/single?name=${imageData.animationFrameOf}`;
  else if (imageData.animationOf != undefined) backUrl = `/single?name=${imageData.animationOf}`;

  // Delete frames first
  if (imageData.isAnimation) await ajaxGet(`/api/animation/delete/${imageData.name}`);

  // Delete upscales as well
  if (imageData.upscales) await ajaxGet(`/api/upscales/delete/${imageData.name}`);
  ...
  if (backUrl != null) reIndexToUrl(backUrl);
  else reindexHome();
}
...
function actionMenuSelection() {
  ...
  if (selectedValue == "make-variations") makeVariations();
  else if (selectedValue == "make-upscale") upscaleFile();
  else if (selectedValue == "make-anim") makeAnimations();
  else if (selectedValue == "download") window.location = `/download/${imageData.name}.png`;
  ...
  } else if (selectedValue == "select-variations") selectVariations();
  else if (selectedValue == "select-upscale") selectUpscale();
  else if (selectedValue == "select-make-anim") selectAnimation();
  else if (selectedValue == "regen-anim") regenAnim();
  else if (selectedValue == "externalize-anim") externalizeAnim();
  else if (selectedValue == "make-extend-anim") extendAnim();
  else if (selectedValue == "select-extend-anim") extendAnim(true);
