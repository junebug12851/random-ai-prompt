...
export default function (argv, settings, imageSettings, upscaleSettings, allSettings) {
  ...
  if (argv.presets !== undefined) {
    ...
    for (let i = 0; i < presets.length; i++) {
      ...
      _.merge(
        allSettings,
        JSON.parse(fs.readFileSync(`${settings.presetFiles}/${preset}.json`, "utf8")),
      );
    }
  }
  ...
  if (argv.chaos !== undefined) {
    ...
    if (settings.deEmphasisChance < 0.25) settings.deEmphasisChance = 0.25;
    else if (settings.deEmphasisChance > 0.5) settings.deEmphasisChance = 0.5;
  ...
  if (
    argv.animationFrames !== undefined &&
    argv.extendAnimationFile === undefined &&
    argv.toAnimationFile === undefined
  ) {
    // Set seed if one isn't set
    if (imageSettings.seed == -1) imageSettings.seed = _.random(100000000, 999999999, false);
  ...
  // If the animation flag is set to extend or convert to an animation
    ...
    argv.animationFrames !== undefined &&
    (argv.extendAnimationFile !== undefined || argv.toAnimationFile !== undefined)
  ) {
    // Set the animation frame count to be the prompt count
    // This can be overridden
    settings.promptCount = parseInt(argv.animationFrames);
  ...
  if (argv.maxCount !== undefined) settings.keywordMaxCount = parseInt(argv.maxCount);

  if (argv.prompts !== undefined) settings.promptCount = parseInt(argv.prompts);

  if (argv.generateImages !== undefined) settings.generateImages = argv.generateImages == true;
  ...
  if (argv.hidePrompt !== undefined) settings.hidePrompt = argv.hidePrompt == true;

  // I don't know why i made so many arguments over the same setting =w=
  if (argv.mode !== undefined) settings.mode = argv.mode;

  if (argv.modeSd == true || argv.modeStableDiffusion == true) settings.mode = "StableDiffusion";

  if (argv.modeMdj == true || argv.modeMidjourney == true) settings.mode = "Midjourney";

  if (argv.modeNai == true || argv.modeNovelai == true) settings.mode = "NovelAI";

  if (argv.emphasis !== undefined) settings.keywordEmphasis = argv.emphasis == true;

  if (argv.emphasisChance !== undefined) settings.emphasisChance = parseFloat(argv.emphasisChance);
  ...
  if (argv.editing !== undefined) settings.keywordEditing = argv.editing == true;

  if (argv.editingMin !== undefined) settings.keywordEditingMin = argv.editingMin;

  if (argv.editingMax !== undefined) settings.keywordEditingMax = argv.editingMax;

  if (argv.alternating !== undefined) settings.keywordAlternating = argv.alternating == true;
  ...
  if (argv.listFiles !== undefined) settings.listFiles = argv.listFiles.toString();

  if (argv.reloadLists !== undefined) settings.reloadListsOnPromptChange = argv.reloadLists == true;
  ...
  if (argv.listEntriesOnce !== undefined) {
    ...
    if (settings.listEntriesUsedOnce == false) settings.reloadListsOnPromptChange = false;
  }

  if (argv.artistFilename !== undefined) settings.artistFilename = argv.artistFilename.toString();

  if (argv.artists !== undefined) settings.artistFilename = argv.artists.toString();

  if (argv.keywordsFilename !== undefined)
    settings.keywordsFilename = argv.keywordsFilename.toString();

  if (argv.dict !== undefined) settings.keywordsFilename = argv.dict.toString();

  if (argv.keywords !== undefined) settings.keywordsFilename = argv.keywords.toString();

  if (argv.expansionFiles !== undefined) settings.expansionFiles = argv.expansionFiles.toString();

  if (argv.presetFiles !== undefined) settings.presetFiles = argv.presetFiles.toString();
  ...
  if (argv.autoArtists !== undefined) settings.autoAddArtists = argv.autoArtists == true;

  if (argv.autoFx !== undefined) settings.autoAddFx = argv.autoFx == true;

  if (argv.promptSalt !== undefined) settings.promptSalt = argv.promptSalt == true;

  if (argv.promptSaltStart !== undefined) settings.promptSaltStart = parseInt(argv.promptSaltStart);

  if (argv.prompt !== undefined) {
    settings.prompt = argv.prompt.toString();
    if (settings.prompt == "true" || settings.prompt == "false") settings.prompt = "";
  }
