...
async function updateProgress() {
  ...
  if (data === undefined && ongoingProgress) {
    ...
    return;
  } else if (!ongoingProgress) return;
...
function startProgress() {
  ...
  samplerProgressBar = progressBars.create(
    imageSettings.steps,
    1,
    {
      samplingStep: 1,
  ...
  batchProgressBar = progressBars.create(
    realbatchCount,
    1,
    {
      jobNo: 1,
  ...
  renderProgressBar = progressBars.create(
    100,
    0,
    {
      sdEta: "--s",
    ...
    },
    {
      format:
        "Image Total: [" +
        "{bar}" +
        "] {percentage}% ETA: {sdEta}, Elapsed: {duration_formatted} {err}",
    },
  );

  promptProgressBar = progressBars.create(
    total,
    index + 1,
    {
      index: index + 1,
...
export default async function (prompt, _index, _total, settings, _imageSettings, upscaleSettings) {
  ...
  // for a single image
  if (imageSettings.width > 512 || imageSettings.height > 512) realbatchCount *= 2;
  ...
  for (let i = 0; i < data.images.length; i++) {
    ...
    if (!settings.upscaleImages) saveName = saveImage(pngBase64, info, imageSettings, false);
    else if (settings.upscaleImages && upscaleSettings.saveBeforeUpscale)
      saveName = saveImage(pngBase64, info, imageSettings, false);
    else if (settings.upscaleImages && !upscaleSettings.saveBeforeUpscale) saveName = false;
