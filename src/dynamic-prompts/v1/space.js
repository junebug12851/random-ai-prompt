...
/**
 * @file
 * @brief Legacy v1 dynamic-prompt generator (#space-v1): a frozen, monolithic version of the scene, kept for reproducibility. See notes/reference/dynamic-prompts.md.
 */
...
function maybeAddColor(fx) {
  ...
  if (_.random(0.0, 1.0, true) < 0.5 && fx) ret += "neon ";

  if (_.random(0.0, 1.0, true) < 0.5 && fx) ret += "glow ";

  if (_.random(0.0, 1.0, true) < 0.5) ret += "{color} ";
...
function addShip() {
  ...
  if (_.random(0.0, 1.0, true) < 0.5) prompt += maybeAddColor(_.random(0.0, 1.0, true) < 0.5);
...
/**
 * Generate the `#space-v1` dynamic-prompt fragment. See notes/reference/dynamic-prompts.md.
...
export default function (settings, imageSettings, upscaleSettings) {
  ...
  if (_.random(0.0, 1.0, true) < 0.5) prompt += ", astronaut";
  else if (_.random(0.0, 1.0, true) < 0.5)
    prompt += personSimple(settings, imageSettings, upscaleSettings);

  if (_.random(0.0, 1.0, true) < 0.5) prompt += `, ${addShip()}`;
  ...
  if (_.random(0.0, 1.0, true) < 0.5) prompt += `, ${maybeAddColor(true)}${maybeAddSize()}sun`;

  if (_.random(0.0, 1.0, true) < 0.5) prompt += `, ${maybeAddColor(true)}${maybeAddSize()}moon`;

  if (_.random(0.0, 1.0, true) < 0.5) prompt += `, ${maybeAddColor(true)}${maybeAddSize()}planet`;
  else if (_.random(0.0, 1.0, true) < 0.5)
    prompt += `, ${maybeAddColor(true)}${maybeAddSize()}{planet}`;

  if (_.random(0.0, 1.0, true) < 0.5) prompt += `, ${maybeAddColor(true)}${maybeAddSize()}star`;
