...
export default async function upscale({ image, key }) {
  ...
  if (!initRes.ok)
    throw new Error(initD?.error || `Leonardo init-image returned ${initRes.status}`);
  const up = initD?.uploadInitImage;
  if (!up?.url || !up?.fields || !up?.id)
    throw new Error("Leonardo did not return an upload target.");
  ...
  const result = await submitPoll({
    ...
    poll: async (sub) => {
      const res = await fetch(`https://cloud.leonardo.ai/api/rest/v1/variations/${sub.id}`, {
        headers,
      });
      return res.json().catch(() => ({}));
    },
    isDone: (s) =>
      (s?.generated_image_variation_generic || []).some((v) => v?.status === "COMPLETE"),
    isFailed: (s) =>
      (s?.generated_image_variation_generic || []).some((v) => v?.status === "FAILED"),
    getImages: (s) =>
      (s?.generated_image_variation_generic || []).map((v) => v.url).filter(Boolean),
    intervalMs: 2500,
    timeoutMs: 180000,
  });
