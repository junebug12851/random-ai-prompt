...
<id>/
  ...
  presets/      provider-owned presets (optional; user presets persist via targets/web/storage)
  code/
...
- `hosted-proxy` — browser → `/api/generate` → upstream (BYOK). Served by the Netlify function
  online and the Vite middleware (`targets/web/vite-plugin-api.js`) locally — both share
  `targets/web/backend/dispatch.js`. e.g. openai.
...
sized to the text provider's (see `targets/web/frontend/lib/home/useImageBatches.js`).
