...
- **`targets/`** — the build targets that consume the engine. **`targets/web/`** is the React/Vite web
  ...
  **`targets/shared/`** is reserved for code shared across targets. **`targets/cli/`** is the
  ...
  unmodified. **`targets/comfyui/`** is the ComfyUI custom-node target (added 2.51.0 — a Python package,
  no npm): **prompt-side only** (ComfyUI owns image gen / upscale / variations / re-roll), it wraps the
  shared engine over HTTP against a running app's backend (**point-at-running-app**; default
  `http://127.0.0.1:4173`), powered by new `/api/prompt` + `/api/prompt/catalog` backend routes that
  reuse `engine/promptRun.js` + `engine/nodeEngine.js` + `engine/presets.js` — no re-ported engine logic.
  The Python nodes are thin HTTP wrappers; a `web/` frontend extension gives live catalog dropdowns. See
  [`notes/systems/comfyui.md`](notes/systems/comfyui.md).
