...
//
// Only used by the Vite SPA. The patterns are relative to THIS file
// (repo-root/core/), reaching the sibling data folders at the repo root.

const dpModules = import.meta.glob("../dynamic-prompts/**/*.js", { eager: true });
const listRaw = import.meta.glob("../lists/*.txt", {
  query: "?raw",
  import: "default",
  eager: true,
});
const expansionRaw = import.meta.glob("../expansions/*.txt", {
  query: "?raw",
  import: "default",
  eager: true,
});
...
export const browserLoader = {
  ...
  readListLines(name) {
    return listLines[name] ?? null;
  },
  listNames() {
    return Object.keys(listLines);
  },
  expansionNames() {
